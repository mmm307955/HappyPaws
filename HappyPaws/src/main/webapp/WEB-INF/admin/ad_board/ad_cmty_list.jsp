	...
	<div class="n_list">
				...
				<c:choose>
            		<c:when test="${ not empty cmtyList}">		
					    <c:forEach var="cmty" items="${cmtyList}">
					    	<tr class="ad_cmty_view" data-seq="${cmty.cmty_seq}" data-count="${cmty.cmty_count}" data-nowpage="${paging.nowPage}" data-searchcondition="${searchCondition}" data-searchkeyword="${searchKeyword}" data-cmty_category="${param.cmty_category}">
					        	<td>${cmty.cmty_category_kr}</td>
					            <td><div class="title-wrapper">${cmty.cmty_title}</div><c:if test="${cmty.comment_count != 0}"><span style="color: #b9b9b9">  [${cmty.comment_count}]</span></c:if></td>
					            <td>${cmty.formattedCmtyDate}</td>
					            <td>
		...
		<ul class="pagination">
			<c:if test="${paging.nowPage > 1 && paging.lastBtn > paging.viewBtnCnt}">
				<li class="page-item"><a class="page-link" href="/admin/ad_cmty_list?nowPage=${paging.nowPage-1}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}&cmty_category=${param.cmty_category}">&lt;</a></li>
			</c:if>
			<c:forEach var="i" begin="${paging.startBtn}" end="${paging.endBtn}" step="1">
				<c:choose>
					...
					<c:otherwise><li class="page-item"><a class="page-link" href="/admin/ad_cmty_list?nowPage=${i}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}&cmty_category=${param.cmty_category}">${i}</a></li></c:otherwise>
				</c:choose>
			</c:forEach>
