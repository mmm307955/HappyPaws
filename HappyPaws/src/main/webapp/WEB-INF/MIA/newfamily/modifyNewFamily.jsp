		...
		<div class="n_write">
			...
			<div class="n_writeform">
				<form action="/updateNewFamily.do" method="post"
					...
					<div class="n_title">
						<input type="text" class="form-control" name="nf_title"
							value="${newFamily.nf_title}" placeholder="제목을 입력하세요." required>
					</div>

					<div class="n_ph">
						<input type="text" class="form-control" name="nf_ph"
							value="${newFamily.nf_ph}" placeholder="연락처를 입력하세요." required>
					</div>
							...
							<tr class="detail-row">
								...
								<td class="value"><select class="form-control"
									name="nf_category" required>
										<option value="dog"
											<c:if test="${newFamily.nf_category == 'M'}">selected</c:if>>남아</option>
										<option value="cat"
											<c:if test="${newFamily.nf_category == 'F'}">selected</c:if>>여아</option>
								</select></td>
							</tr>
							<tr class="detail-row">
								...
								<td class="value"><select class="form-control"
									name="nf_category" required>
										<option value="dog"
											<c:if test="${newFamily.nf_category == 'dog'}">selected</c:if>>강아지</option>
										<option value="cat"
											<c:if test="${newFamily.nf_category == 'cat'}">selected</c:if>>고양이</option>
										<option value="small"
											<c:if test="${newFamily.nf_category == 'small'}">selected</c:if>>소동물</option>
										<option value="etc"
											<c:if test="${newFamily.nf_category == 'etc'}">selected</c:if>>기타</option>
								</select></td>
							</tr>
							<tr class="detail-row">
								<td class="label">품종</td>
								<td class="value"><input type="text" class="form-control"
									name="nf_breed" value="${newFamily.nf_breed}" required></td>
							</tr>
							<tr class="detail-row">
								...
								<td class="value"><select class="form-control"
									name="nf_ok" required>
										<option value="N"
											<c:if test="${newFamily.nf_ok == 'N'}">selected</c:if>>분양 중이에요</option>
										<option value="Y"
											<c:if test="${newFamily.nf_ok == 'Y'}">selected</c:if>>분양 완료했어요</option>
								</select></td>
							</tr>
						</table>
					</div>
					...
					<div class="input-group mb-3">
						<textarea class="form-control" rows="5" name="nf_content"
							placeholder="상세 설명을 입력하세요">${newFamily.nf_content}</textarea>
					</div>
