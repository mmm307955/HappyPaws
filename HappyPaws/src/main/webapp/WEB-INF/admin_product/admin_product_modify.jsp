        ...
        <form action="ad_manageProductModify" method="POST" enctype="multipart/form-data"
                    ...
                    <label for="pr_status">
                        <select id="pr_status" name="pr_status">
                            <option value="available"
                                    <c:if test="${product.pr_status == 'available'}">selected</c:if>>
                                판매중
                            </option>
                            <option value="out_of_stock"
                                    <c:if test="${product.pr_status == 'out_of_stock'}">selected</c:if>>
                                일시품절
                            </option>
                            <option value="discontinued"
                                    <c:if test="${product.pr_status == 'discontinued'}">selected</c:if>>
                                품절
                            </option>
                        </select>
                        <br>
            ...
            <div class="form-group">
                <label for="pr_price">대표 가격(원)
                    <input type="number" id="pr_price" name="pr_price" value="${product.pr_price}"
            ...
            <div class="form-group">
                <label for="pr_category">카테고리 선택
                    <select id="pr_category" name="pr_category">
                        <option value="식품"
                                <c:if test="${product.pr_category == '식품'}">selected</c:if>>식품
                        </option>
                        <option value="위생"
                                <c:if test="${product.pr_category == '위생'}">selected</c:if>>위생
                        </option>
                        <option value="미용"
                                <c:if test="${product.pr_category == '미용'}">selected</c:if>>미용
                        </option>
                        <option value="의류"
                                <c:if test="${product.pr_category == '의류'}">selected</c:if>>의류
                        </option>
                        <option value="놀이"
                                <c:if test="${product.pr_category == '놀이'}">selected</c:if>>놀이
                        </option>
                    </select>
                </label>
            </div>
                ...
                <c:forEach var="option" items="${productOption}">
                    <div class="option-group">
                        <div class="option-name_status">
                            ...
                            <input type="text" name="pr_opt_name" id="pr_opt_name"
                                   value="${option.pr_opt_name}" placeholder="옵션명 필수 입력">
                            <label for="pr_opt_status">
                                <select id="pr_opt_status" name="pr_opt_status">
                                    <option value="available"
                                            <c:if test="${option.pr_opt_status == 'available'}">selected</c:if>>
                                        판매중
                                    </option>
                                    <option value="out_of_stock"
                                            <c:if test="${option.pr_opt_status == 'out_of_stock'}">selected</c:if>>
                                        일시품절
                                    </option>
                                    <option value="discontinued"
                                            <c:if test="${option.pr_opt_status == 'discontinued'}">selected</c:if>>
                                        품절
                                    </option>
                                </select>
                            </label>
                        ...
                        <div class="option-stock_price">
                            <label for="pr_opt_price">
                                <input type="number" name="pr_opt_price" id="pr_opt_price"
                                       value="${option.pr_opt_price - product.pr_price}"
                                       placeholder="옵션 추가금 입력">
                            </label>
                            <label for="pr_opt_stock">
                                <input type="number" name="pr_opt_stock" id="pr_opt_stock"
                                       value="${option.pr_opt_stock}" placeholder="재고 수량 입력">
                            </label>
            ...
            <div class="buttons">
                <button type="button"
                        onclick="cancel(${nowPage}, '${searchCondition}', '${searchKeyword}', '${param.category}')">
                    취소
                </button>

                <button type="submit">수정</button>
                <button onclick="deleteProduct('${product.pr_id }',${nowPage}, '${searchCondition}', '${searchKeyword}', '${param.category}')">
                    삭제
                </button>
            </div>
...
<script>
  ...
  function deleteProduct(pr_id, nowPage, searchCondition, searchKeyword, category) {
    if (confirm("정말로 삭제하시겠습니까?")) {
      location.href = "/deleteProduct?pr_id=" + pr_id + "&nowPage=" + nowPage + "&searchCondition="
          + searchCondition + "&searchKeyword=" + searchKeyword + "&pr_category=" + category;
    }
  }
    ...
    $('#editor').on('click', 'img', function () {
      ...
      $handle.on('mousedown touchstart', function (e) {
        ...
        $(document).on('mousemove.resize touchmove.resize', function (event) {
          ...
          var newWidth = Math.min(450, Math.max(50, currentX - $img.offset().left));
      ...
      $(document).on('click.removeElements', function (e) {
        if (!$(e.target).is($img) && !$(e.target).is($alignContainer) && !$(e.target).closest(
