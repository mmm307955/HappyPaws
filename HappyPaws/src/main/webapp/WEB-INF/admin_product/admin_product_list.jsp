    ...
    <div class="main-content">
        <div class="n_seachform">
            <form>
                ...
                <label style="display: none;"><select name="searchCondition">
                    <option value="TITLE" ${searchCondition == 'TITLE' ? 'selected' : ''} >제목
                    </option>
                    <option value="CONTENT" ${searchCondition == 'CONTENT' ? 'selected' : ''}>내용
                    </option>
                </select></label>
                <div class="custom-select-wrapper">
                    ...
                    <label>
                        ...
                        <button type="submit"><img
                                src="${pageContext.request.contextPath}/resources/images/searchicon.png"
                                alt="검색" title="검색"></button>
                    </label>
                </div>
            </form>
        </div>
            ...
            <c:choose>
                <c:when test="${not empty productsList}">
                    <c:forEach items="${productsList}" var="products">
                        <tr onclick="selTr(${products.pr_id}, '${searchCondition}', '${searchKeyword}','${category}', ${paging.nowPage})"
                            style="cursor: pointer;">

                            <td class="tdCenter">${products.pr_id}</td>
                            <td class="tdCenter" style="text-align: center;"><img
                                    src="${pageContext.request.contextPath}/resources/upload/${products.pr_thumbnail}"
                                    alt="썸네일 이미지" style="height: 50px;"
                                    onerror="this.src='../../../resources/images/HappyPawsLogo.png';">
                            </td>

                            <td class="tdCenter">${products.pr_name}</td>
                            <td class="tdCenter"><fmt:formatNumber value="${products.pr_price}"
                                                                   pattern="#,###"/></td>
                                    ...
                                    <c:forEach var="opt" items="${allOpt}">
                                        <c:if test="${products.pr_id eq opt.pr_id}">
                                            <ul style="list-style-type: none; padding: 0; color:black;">
                                                ...
                                                <li>재고: <fmt:formatNumber
                                                        value="${opt.pr_opt_stock}"
                                                        pattern="#,###"/> 개
                                                </li>
                                                <li>금액: <fmt:formatNumber
                                                        value="${opt.pr_opt_price}"
                                                        pattern="#,###"/> 원
                                                </li>
                                            </ul>
                                        </c:if>
        ...
        <ul class="pagination">
            <c:if
                ...
                <li class="page-item"><a class="page-link"
                                         href="ad_manageProductList?nowPage=${paging.nowPage-1}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}&category=${category}">이전</a>
                </li>
                </li>
            </c:if>
            <c:forEach var="i" begin="${paging.startBtn}" end="${paging.endBtn}"
                ...
                <c:choose>
                    ...
                    <c:otherwise>
                        <li class="page-item"><a class="page-link"
                                                 href="ad_manageProductList?nowPage=${i}&searchCondition=${searchCondition}&searchKeyword=${searchKeyword}&category=${category}">${i}</a>
                        </li>
                        </li>
                    </c:otherwise>
                </c:choose>
        ...
        <script>
          document.addEventListener("DOMContentLoaded", function () {
            let currentlyOpenPopup = null; // 현재 열려 있는 팝업 추적
